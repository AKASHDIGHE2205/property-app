...
import { getActiveConsignee } from "../../../services/Property/transaction/pTranApis";
import { useDispatch } from "react-redux";
import { handlePConsignee } from "../../../feature/Preperties/ptransaction/PTransactionSlice";

interface Props {
  show: boolean;
  setShow: (show: boolean) => void;
}
const ConsigneeModal: FC<Props> = ({ show, setShow }) => {
  ...
  const handleSelect = (item: any) => {
    dispatch(handlePConsignee({ id: item.id, name: item.name }))
    setShow(false);
  }
  ...
  return (
    <div
      ...
      role="dialog"
      aria-labelledby="Consignee-modal-label"
    >
      <div className="flex justify-center items-center min-h-screen m-2">
        <div className="w-full max-w-lg bg-white dark:bg-slate-800 rounded-xl shadow-sm overflow-hidden">
          <div className="flex justify-between items-center py-3 px-4 border-b dark:border-slate-700">
            <h3 id="Consignee-modal-label" className="font-bold text-slate-800 dark:text-white">
              Select Consignee
            </h3>
            <button
              type="button"
              ...
              <tbody>
                {loading ?
                  ...
                  : (<>
                    ...
                    <tr>
                      <td colSpan={13} className="text-center py-4">{filtereData?.length === 0 ? "No consignee found" : null}</td>
                    </tr>
                  </>)}
              </tbody>
            </table>
