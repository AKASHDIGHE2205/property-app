...
import { getAllConsignees } from "../../../../services/Property/master/PmasterApis"
import ConsigneeCreate from "./ConsigneeCreate"
import ConsigneeEdit from "./ConsigneeEdit"
...
const ConsigneeView = () => {
  ...
  const fetchData = async () => {
    setLoading(true)
    try {
      const response = await getAllConsignees();
      setData(response);
    } catch (error) {
  ...
  return (
    <div className="min-h-screen w-full bg-slate-50 dark:bg-slate-900 p-4 md:p-6">
      <div className="max-w-7xl mx-auto bg-white dark:bg-slate-800 rounded-xl shadow-lg overflow-hidden border border-slate-200 dark:border-slate-700">
        ...
        {/* Action Bar */}
        <div className="p-4 md:p-6 border-b border-slate-200 dark:border-slate-700 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
          {/* Search Input */}
                  ...
                  <tr>
                    <td colSpan={4} className="px-6 py-4 text-center">
                      <div className="flex justify-center items-center space-x-2">
                        ...
                        <span className="text-slate-600 dark:text-slate-400">Loading consignees...</span>
                      </div>
                    </td>
                  </tr>
                  ...
                  <tr>
                    <td colSpan={4} className="px-6 py-4 text-center">
                      <div className="flex flex-col items-center justify-center py-8">
                        <FiUser className="text-slate-400 text-4xl mb-2" />
                        <p className="text-slate-600 dark:text-slate-400 text-lg">No consignees found</p>
                        <p className="text-slate-500 dark:text-slate-500 text-sm">Try adjusting your search or add a new consignee</p>
                      </div>
                    </td>
                  </tr>
                )}
