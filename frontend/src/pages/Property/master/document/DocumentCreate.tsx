...
import { newDocument } from "../../../../services/Property/master/PmasterApis";
...
const DocumentCreate: FC<Props> = ({ show, setShow, fetchData }) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const response = await newDocument(body);
      if (response) {
        setShow(false);
        fetchData()
      }
  ...
  return (
    <div
      ...
      role="dialog"
      aria-labelledby="Document-create-modal-label"
    >
      <div className="flex justify-center items-center min-h-screen m-2">
        <div className="w-full max-w-lg bg-white dark:bg-slate-800 rounded-xl shadow-sm overflow-hidden">
          <div className="flex justify-between items-center py-3 px-4 border-b dark:border-slate-700">
            <h3 id="Document-create-modal-label" className="font-bold text-slate-800 dark:text-white">
              Add Document
            </h3>
            <button
              type="button"
          ...
          <form onSubmit={handleSubmit}>
            <div className="mx-3 my-2">
              <label htmlFor="DocumentName">
                Document Name: <span className="text-red-600 text-xl">*</span>
              </label>
              <input
                type="text"
                name="name"
