...
import { getActiveCatgs } from "../../../../services/Stores/transaction/transactionApi";
import { useDispatch } from "react-redux";
import { handleCategory } from "../../../../feature/Stores/transaction/TransactionSlice";
...
const CategoryModal: FC<Props> = ({ show, setShow }) => {
  ...
  const fetchData = async () => {
    setLoading(true);
    try {
      const response = await getActiveCatgs();
      setData(response);
    } catch (error) {
  ...
  const handleSelect = (item: any) => {
    dispatch(handleCategory({ id: item.id, name: item.name }));
    setShow(false);
  };
