...
import { getActiveLocations } from "../../../../services/Stores/transaction/transactionApi";
import { useDispatch } from "react-redux";
import { handleLocation } from "../../../../feature/Stores/transaction/TransactionSlice";
...
const LocationModal: FC<props> = ({ show, setShow }) => {
  ...
  const handleSelect = (item: any) => {
    dispatch(handleLocation({ id: item.id, name: item.name }))
    setShow(false)
  }
  ...
  return (
    <div
      ...
      role="dialog"
      aria-labelledby="loc-modal-label"
    >
      <div className="flex justify-center items-center min-h-screen m-2">
        <div className="w-full max-w-lg bg-white dark:bg-slate-800 rounded-xl shadow-sm overflow-hidden">
          <div className="flex justify-between items-center py-3 px-4 border-b dark:border-slate-700">
            <h3 id="loc-modal-label" className="font-bold text-slate-800 dark:text-white">
              Select Location
            </h3>
            <button
              type="button"
          ...
          <div className="px-4 max-h-[70vh] overflow-y-auto">
            ...
            <table className="min-w-full divide-y divide-slate-200 dark:divide-slate-700">
              <thead className="sticky top-0 bg-slate-50 dark:bg-slate-900">
                <tr>
                  <th className="px-2 py-3 text-start text-xs font-medium text-slate-500 uppercase">Location Code</th>
                  <th className="px-2 py-3 text-start text-xs font-medium text-slate-500 uppercase">Location Name</th>
