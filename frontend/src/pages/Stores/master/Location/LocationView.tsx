...
import { getAllLocation } from "../../../../services/Stores/master/MasterApis";
  ...
  return (
    <>
      <div className="h-screen w-full m-3 border p-2 rounded-lg">
        <h1 className="flex justify-center items-center text-2xl font-semibold ">Location Details</h1>
          ...
          <div className="border border-gray-300 rounded-md">
            <div className="overflow-y-auto overflow-x-auto max-h-[30rem]">
              <table className="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
                <thead className="bg-gray-100 dark:bg-gray-800 static top-0">
                  <tr>
                    ...
                    {currentItems?.map((item: any) => (
                      <tr key={item.id} className="uppercase">
                        ...
                        <td className={tablebody}>
                          <button
                            type="button"
                            className="py-2 px-2 items-center gap-x-2 text-sm font-medium rounded-lg border border-transparent bg-blue-100 
                          text-[#0015FF] hover:bg-blue-200 focus:outline-none focus:bg-blue-200 disabled:opacity-50 disabled:pointer-events-none dark:text-[#52baff] dark:bg-blue-800/30 dark:hover:bg-blue-800/20 dark:focus:bg-blue-800/20"
      ...
      {showEditModal && <EditLocation show={showEditModal} setShow={setShowEditModal} fetchData={fetchData} selectedLoc={selectedLoc} />}
      {showCreateModal && <CreateLocation show={showCreateModal} setShow={setShowCreateModal} fetchData={fetchData} />}
    </>
  )
}
