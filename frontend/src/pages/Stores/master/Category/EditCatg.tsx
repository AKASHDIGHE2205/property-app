...
import { editCatg } from "../../../../services/Stores/master/MasterApis";
...
const EditCatg: FC<props> = ({ show, setShow, fetchData, selectedCatg }) => {
  ...
  useEffect(() => {
    if (selectedCatg) {
      setInput({
        id: selectedCatg.id || 0,
        name: selectedCatg.name || "",
        status: selectedCatg.status || "",
      });
    }
  }, [selectedCatg]);
  ...
  return (
    <div
      ...
      role="dialog"
      aria-labelledby="catg-create-modal-label"
    >
      <div className="flex justify-center items-center min-h-screen m-2">
        <div className="w-full max-w-lg bg-white dark:bg-slate-800 rounded-xl shadow-sm overflow-hidden">
          <div className="flex justify-between items-center py-3 px-4 border-b dark:border-slate-700">
            <h3 id="catg-create-modal-label" className="font-bold text-slate-800 dark:text-white">
              Edit Firm
            </h3>
            <button
              type="button"
